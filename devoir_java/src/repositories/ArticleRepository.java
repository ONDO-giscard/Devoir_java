...
public class ArticleRepository extends Database {
    private final String SQL_INSERT_ARTICLE = "INSERT INTO `article` (`titre`, `contenu`, `dateCreation`, `etat`, `id_cat`) VALUES (?, ?, ?, ?, ?)";
    private final String SQL_SELECT_ALL_ARTICLES = "SELECT a.*, c.nomCat FROM article a INNER JOIN categorie c ON a.id_cat = c.id_cat";
